...
class SweepLine:
    ...
    def _check_intersection(self, a: Event, b: Event):
        ...
        if USE_IGNORE_SEGMENT_ENDINGS:
            if (point_len_sq(a.segment[0], p) < NUM_EPS_SQ
                or point_len_sq(a.segment[1], p) < NUM_EPS_SQ
                or point_len_sq(b.segment[0], p) < NUM_EPS_SQ
                or point_len_sq(b.segment[1], p) < NUM_EPS_SQ):
                return
    ...
    def handle(self, p, events_current):
        ...
        assert p[X] == self._current_event_point_x
...
def shortest_line_between_segments(A, B, C, D):
    """
    Finds the shortest line segment between two line segments AB and CD in 2D.
    
    Parameters:
        A, B, C, D: tuple[float, float], endpoints of the line segments (2D points).

    Returns:
        P1, P2: nparray[float, float], closest points on segments AB and CD respectively.
        min_dist: float, minimum distance between the segments.
    """

    A = np.array(A); B = np.array(B); C = np.array(C); D = np.array(D)
    
    def segment_point(t, P1, P2):
        """Returns the point on a segment given t in [0, 1]."""
        return (1 - t) * P1 + t * P2
    
    def distance(t, u):
        """Computes the squared distance between P(t) and Q(u)."""
        P_t = segment_point(t, A, B)
        Q_u = segment_point(u, C, D)
        return np.sum((P_t - Q_u) ** 2)

    # Numerical optimization bounds
    bounds = [(0, 1), (0, 1)]  # t in [0, 1], u in [0, 1]
    ...
    result = minimize(
        ...
        x0=[0.5, 0.5],  # Start with midpoints
        bounds=bounds
    )
    
    # Get the optimal t and u
    t_opt, u_opt = result.x

    # Compute the closest points on each segment
    P1 = segment_point(t_opt, A, B)
    P2 = segment_point(u_opt, C, D)
...
def point_len_sq(A: np.ndarray, B: np.ndarray):
    if type(A) is not np.ndarray: A = np.array(A)
    if type(B) is not np.ndarray: B = np.array(B)
    vect: np.ndarray = B - A
    return vect.dot(vect)

def isect_seg_seg_v2_point(A, B, C, D, bias=NUM_ZERO):
    ...
    P1, P2, min_dist = shortest_line_between_segments(A, B, C, D)
    if min_dist < DISTANCE_THRESHOLD:
        return tuple(((P1 + P2) / 2).tolist())
    else:
        return None
        ...
        for j in range(i + 1, n):
            ...
            if a0 not in (b0, b1) and a1 not in (b0, b1):
                ...
                if ix is not None:
                    ...
                    if USE_IGNORE_SEGMENT_ENDINGS:
                        if (point_len_sq(a0, ix) < NUM_EPS_SQ
                            or point_len_sq(a1, ix) < NUM_EPS_SQ
                            or point_len_sq(b0, ix) < NUM_EPS_SQ
                            or point_len_sq(b1, ix) < NUM_EPS_SQ):
                            continue
